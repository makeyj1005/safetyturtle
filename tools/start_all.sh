...
# [2026-09-06 최종] 1280x720 @ 8fps.
#
# 전압 부족 때문에 320x240 까지 낮췄더니 **안전모 판정이 망가졌다** —
# YOLO 점수가 -0.65 로 나와 착용 중인데도 미착용으로 잡혔다.
# 1280x720 으로 되돌리니 +0.906 으로 확실하게 잡힌다.
# (13장 중 미착용 2장 -> 착용 판정. 낮은 해상도에서는 10장 중 4장이었다)
#
# 전압 여유는 CSI 를 640x480 으로 낮춘 것으로 확보했다. 진짜 원인은
# CSI 1640x1232 였다 — 그걸 켜는 순간 파이가 죽었다.
# fps 는 8 로 둔다. 안전모 판정은 2초 창에서 하므로 8fps 면 충분하고,
# 25fps 로 올리면 파이가 인코딩에 코어 하나를 다 쓴다.
...
# --- CSI(후면) 화질 ---
# [2026-09-06 최종] 1640x1232 로 되돌렸다.
#
# 한때 640x480 까지 낮췄다. 노드를 4초 간격으로 하나씩 올려보니 GPIO·스피커·
# LCD·웹캠은 전부 통과하고 CSI(1640x1232)를 켜는 순간 파이가 전압 부족으로
# 죽었기 때문이다(재현 2회). 그때는 Nav2 까지 돌고 있었다.
#
# Nav2 를 끄고 나니 여유가 생겨 1640x1232 로도 전압부족 0/34 회로 안정적이다.
# 이 해상도가 필요한 이유: 압력계 게이지가 약 48px 이 되어야 바늘을 볼 수 있고
# (640x480 이면 15px), gauge.judge 는 캘리브레이션 해상도와 다르면 아예 거부한다.
# 1640x1232 는 CMA 상한이기도 하다(그 위는 "Cannot allocate memory").
#
# ⚠️ 자율주행(Nav2)을 함께 돌리면 전압이 모자란다. 그때는 640x480 으로 낮출 것:
#      CSI_W=640 CSI_H=480 ./tools/start_all.sh
CSI_W="${CSI_W:-1640}"
CSI_H="${CSI_H:-1232}"
CSI_FPS="${CSI_FPS:-5}"
...
# ---------------- Fast-DDS 프로파일 (기본값: 안 씀)
# [2026-09-07 발표장] 어제 만든 인터페이스 화이트리스트 프로파일이 이 망에서는
# 오히려 통신을 막았다. 프로파일을 빼고 기본 설정으로 하니 전부 정상이다
# (/battery 20Hz, /scan 5Hz, /webcam 6.3fps).
#
# 어제 이 프로파일이 필요했던 이유는 docker0(172.17.0.1)로 발견 패킷이
# 새는 것을 막기 위해서였다. 망이 바뀌면 필요 여부도 바뀌므로 기본은 끈다.
# 필요하면 환경변수로 켤 것:
#   USE_DDS_PROFILE=1 ./tools/start_all.sh
DDS_ENV=()
if [ "${USE_DDS_PROFILE:-0}" = "1" ]; then
    ...
    if [ -n "$WIFI_IP" ] && [ -f "$EX1/tools/fastdds_wifi.xml" ]; then
        mkdir -p "$EX1/logs/control"
        sed "s/__WIFI_IP__/$WIFI_IP/" "$EX1/tools/fastdds_wifi.xml" > "$EX1/logs/control/fastdds.xml"
        DDS_ENV=(-e "FASTRTPS_DEFAULT_PROFILES_FILE=/root/vibe/ex1/logs/control/fastdds.xml")
        echo "  DDS 프로파일 사용: $WIFI_IP"
    fi
fi
