...
# --- CSI(후면) 화질 ---
# [2026-09-06] 1640x1232 -> 640x480 으로 낮췄다. **이게 파이를 죽이던 범인이다.**
# 노드를 하나씩 올려보니 GPIO·스피커·LCD·웹캠은 전부 통과하고 CSI 를 켜는
# 순간 파이가 전압 부족으로 죽었다(재현 2회). 640x480 으로 낮추니
# 전압부족 0/21 회로 완전히 안정됐다.
#
# 대가: 압력계 판정을 못 한다. 그 거리에서 게이지가 약 15px 라 바늘이 안 보이고,
# gauge.judge 는 캘리브레이션 해상도(1640x1232)와 다르면 아예 거부한다.
# QR 인식과 대장 조회(제조년월·교체년월·책임자)는 이 해상도로도 잘 된다.
#
# 압력계까지 살리려면 파이에 별도 5V 전원(USB-C)을 물리고 OpenCR->파이
# 전원선을 빼면 된다. 그러면 1640x1232 로 되돌릴 수 있다.
CSI_W="${CSI_W:-640}"
CSI_H="${CSI_H:-480}"
CSI_FPS="${CSI_FPS:-5}"
